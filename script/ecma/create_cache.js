...
const extractVariables = () => {
  const arr = PPx.Extract('%n,%*path,%*lfdir%\\,%*base').split(',');
  const drive = arr[1].split(':')[0];
  const listfile = drive === '' ? 'root.xlf' : `${arr[1].replace(/[:\/]/g, '_')}.xlf`;
  const root = `${arr[3].split(' ')[0]}${drive}:`;
 ...
 */
const extractOn = (ppcid, target) => PPx.Extract(`%*extract(${ppcid},"${target}")`);
 ...
 */
const hasPPb = (ppcid) => {
  const ppbid = extractOn(ppcid, `%%si'${rc.name.ppb}'`);
  const idlist = PPx.Extract('%*ppxlist(B)').replace(/_/g, '');
 ...
 * @param {string} passcmd
 */
const rcloneSync = (args, data, passcmd) => {
 ...
 * @param {string} passcmd
 */
const rcloneAsync = (args, data, passcmd) => {
  const spin = args.spin !== '0';
  const spin_count = '300';
  let endjob;
  ...
  const ppbid = extractOn(data.ppcid, `%%si'${rc.name.ppb}'`);

  PPx.Execute(
    `*execute ${ppbid},*run -wait:later ${lsToLf(data, passcmd)} %(
	*wait -run
	*string p,${rc.name.err}=%*exitcode
	*execute ${data.ppcid},${endjob}
	*wait ${spin_count},2
	*if 0==%sp'${rc.name.err}'%:*execute ${data.ppcid},*jumppath aux://S_auxRCLONE/${data.path} -savelocate
	*if 1==%sp'${rc.name.err}'%:*execute ${data.ppcid},*string p,${rc.name.passcmd}=%%:*linemessage !"${rc.mes.cancel}
	*if 1<%sp'${rc.name.err}'%:*execute ${data.ppcid},*linemessage !"${rc.mes.error}
	*string p,${rc.name.err}=
	*execute ${data.ppcid},*string p,${rc.key}=%)`
  );
...
const isAsync = (async, path) => {
  ...
  const ppmsync = PPx.Extract('%si"rc_sync"');
...
/* Main */
const gArgs = adjustArg(PPx.Arguments);
const gData = extractVariables();
...
const passCmd = rc.createPasscmd(gArgs.decrypt);
